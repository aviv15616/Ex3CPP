            ...
            try {
                ...
                if(target_name==current->get_name()) {
                   target->ensure_coup_required();
                    ...
                    if (!gov_real) throw std::runtime_error("Player is not a Governor");
                    ...
                    for (const auto &[player_name, action] : last_actions) {
                        if (action == "tax" && game.can_still_undo(player_name) && player_name != target_name) {
                            ...
                            if (p && p->is_active()) tax_targets.push_back(p);
                        }
                    }

                    if (!tax_targets.empty()) {
                        auto selected = show_selection_popup(tax_targets, "Choose Player to undo tax for:", sf::Color(70, 70, 200));
                        if (selected) gov_real->undo_tax(*selected);
                        else info_message = "No target selected.";
                    } else error_message = "No tax targets available.";
                }

                else if (role == "Judge") {
                    auto *judge_real = dynamic_cast<Judge *>(target.get());
                    if (!judge_real) throw std::runtime_error("Player is not a Judge");
                    ...
                    if (!general_real) throw std::runtime_error("Player is not a General");
                    ...
                    if (!coup_targets.empty()) {
                        ...
                        if (selected) general_real->undo_coup(*selected);
                        else info_message = "No target selected.";
                    } else error_message = "No coup targets available.";
                }

                else if (role == "Spy") {
                    auto *spy_real = dynamic_cast<Spy *>(target.get());
                    if (!spy_real) throw std::runtime_error("Player is not a Spy");
...
bool GUI::handleTargetActionClick(const sf::Vector2i &mouse, std::shared_ptr<Player> current) {
    if (pending_target_action == PendingTargetAction::None) return false;
    ...
    return false;  // ❌ לא בוצעה לחיצה על יעד — נחזיר false אבל לא נבטל עדיין
}
        ...
        if (pair.first.contains(static_cast<float>(mouse.x), static_cast<float>(mouse.y))) {
            try {
                if (pair.second == "Gather") {
                    ...
                    if (!baron) throw InvalidActionException("Only a Baron can use Invest.");
                    baron->invest();
                }
                    ...
                    current->skip_turn();
                    info_message = current->get_name() + " skipped their turn.";
                    render();
                    return true;
                }
